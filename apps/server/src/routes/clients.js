...
async function clientRoutes(fastify) {
    ...
    fastify.post('/clients', async (request, reply) => {
        try {
            ...
            if (body.id) {
                const existing = await db_1.prisma.client.findUnique({ where: { id: body.id } });
                if (existing) {
                    const updated = await db_1.prisma.client.update({
                        where: { id: body.id },
                        data: {
            ...
            if (!clientCode) {
                const allClients = await db_1.prisma.client.findMany({ select: { code: true } });
