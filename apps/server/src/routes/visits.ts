...
import { VisitSchema, VisitStatus } from '@sistema-os/shared';
import { notifyVisitStatusUpdate } from '../socket';
...
export async function visitRoutes(fastify: FastifyInstance) {
  ...
  fastify.get('/visits', async (request, reply) => {
    const { date, period, technician } = request.query as {
      date?: string;
      period?: string;
      technician?: string;
    };
  ...
  fastify.post('/visits', async (request, reply) => {
    ...
    const visit = await prisma.visit.create({
      data: {
        ...
        status: body.status || VisitStatus.AGENDADA,
  ...
  fastify.patch('/visits/:id/status', async (request, reply) => {
    ...
    const { status, notes, photos, signatureUrl, partsUsed } = request.body as {
      status: VisitStatus;
      notes?: string;
      photos?: string[];
      signatureUrl?: string;
      partsUsed?: Array<{ name: string; code?: string; price: number; quantity: number }>;
    };
    ...
    if (partsUsed && partsUsed.length > 0) {
      await prisma.partUsed.deleteMany({ where: { visitId: id } });
